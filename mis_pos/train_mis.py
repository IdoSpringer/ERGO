...
import mis_pos.ae_mis_pos as ae
import pickle
import os
...
def lstm_get_lists_from_pairs(pairs, mis):
    tcrs = []
    peps = []
    signs = []
    for pair in pairs:
...
def train_model(args):
    ...
    arg['test_auc_file'] = 'mis_pos/mis_pos_auc/' + '_'.join([args.model_type, args.dataset,
                                                              'test', str(iter), str(mis)])
    ...
    if args.model_type == 'ae':
        ...
        train_batches = ae.get_batches(train_tcrs, train_peps, train_signs, tcr_atox, pep_atox, params['batch_size'],
                                       params['max_len'], mis)
        # test
        test_tcrs, test_peps, test_signs = ae_get_lists_from_pairs(test, params['max_len'])
        test_batches = ae.get_batches(test_tcrs, test_peps, test_signs, tcr_atox, pep_atox, params['batch_size'],
                                      params['max_len'], mis)
        # Train the model
    ...
    if args.model_type == 'lstm':
        # train
        train_tcrs, train_peps, train_signs = lstm_get_lists_from_pairs(train, mis)
        ...
        # test
        test_tcrs, test_peps, test_signs = lstm_get_lists_from_pairs(test, mis)
